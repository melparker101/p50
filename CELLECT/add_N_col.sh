...
# Try this so that the colname for direction doesn't split
# awk 'NR==1 {print $1 "hello", "hello2", "hello3",; next} NR==FNR -F" " '{printf "%s\t", $1; gsub(/.{1}/,"& ",$2); print $2}' "$ORIG_SUMSTATS"/Infertility1_F_EUR_directions.txt > tmp && mv tmp "$ORIG_SUMSTATS"/Infertility1_F_EUR_directions.txt
